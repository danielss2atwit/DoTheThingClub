...
export default function IntakeOverlay({
  ...
  let intakeQ = '';
  let intakeHelp = '';
  let intakeIsText = false;
  let intakeIsChips = false;
  let intakeIsSuggest = false;
  let intakePlaceholder = '';
  let intakeChips = [];

  if (stepName === 'thing') {
    intakeQ = 'What do you want to get braver about?';
    intakeHelp = 'Not a whole personality change — just one comfort zone. One or two sentences is plenty.';
    intakeIsText = true;
    intakePlaceholder = 'e.g. Speaking up in class without overthinking it…';
  } else if (stepName === 'field') {
    intakeQ = 'Which area is this in?';
    intakeHelp = 'Helps us match you with near-peers working on the same kind of brave.';
    intakeIsChips = true;
    intakeChips = fieldOptions.map((f) => ({ label: f, on: intakeField === f, onClick: () => onSetField(f) }));
    ...
    intakeHelp = 'We turned your idea into 3 tiny, low-stakes first steps.';
              ...
              <>
                {suggestLoading && (
                  <div style={{ marginTop: 34, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
                    ...
                    <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--lilac,#6562ac)' }}>Breaking your idea into first steps…</div>
                  </div>
                )}
                {!suggestLoading && (
